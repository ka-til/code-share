...
#14)POCAM_num
    ...
    #Time residual graph is very sensitive to the POCAM interval
    	if POCAM_num ==['P1']:
    		self.estimate_peak = 200100.71     #an approximation of POCAM interval for POCAM_1
    	if POCAM_num == ['P2']:
    		self.estimate_peak = 200100.33     #an approximation of POCAM interval for POCAM_2
...
#3)med_bound decides width of the gaussian formed when time residual histogram is plotted.  
    def res(self, gaus_peak, greater_than, med_bound):
        ...
        jump_index = np.append(jump_index, 0) #appending zero jump_index
    ...
    def calc_res(self, BinsHist, med_bound, yaxis_lbound, yaxis_ubound):
    	...
    	for d in range(0, self.JumpIndex.size + 1):
        	...
        	if d == self.JumpIndex.size:
        		...
        		print('yaxis - ', y_axis)
        		lower_bound = y_axis -  yaxis_lbound #defining the width of the selection of events along y-axis(time residuals)
        		upper_bound = y_axis + yaxis_ubound #defining the width of the selection of events along y-axis(time residuals)
        		time_res = select[(select >= self.yedges[lower_bound]) & (select <= self.yedges[upper_bound])] #selecting timestamp events that have the defined time residual value
        	else:
        		b = self.JumpIndex[d] 
